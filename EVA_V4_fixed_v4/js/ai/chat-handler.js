...
// ═══ SANITIZE LLM OUTPUT ═══
// Les petits modèles (Llama-3.2-3B, etc.) hallucinent parfois des préfixes parasites
// du genre "cyan actif", "[système : cyan actif]", "*cyan actif*", "outil cyan actif",
// "tool: ...", "assistant:", etc. avant la vraie réponse. On les retire défensivement.
function _stripLeakedPrefix(text) {
  if (!text || typeof text !== 'string') return text;
  let out = text;
  // Retirer jusqu'à 3 préfixes parasites (un peu paranoïaque, on enchaîne)
  for (let i = 0; i < 3; i++) {
    const before = out;
    // Préfixes typiques en début de réponse, optionnellement entourés de [], (), **, *,  ou suivis de :
    out = out.replace(
    ...
    // Retirer une éventuelle ligne d'en-tête en markdown laissée vide
    out = out.replace(/^\s*\n+/, '');
    if (out === before) break;
  }
  return out.trim() || text; // si on a tout vidé par erreur, on rend l'original
}
...
async function sendMessage(userMessage, options) {
  ...
  try {
    ...
    // Nettoyage défensif des préfixes parasites genre "cyan actif"
    const cleaned = _stripLeakedPrefix(response.content);
